...
export class IndexComponent implements OnInit {
  ...
  searchPosts(theme: string): void{
    if(theme === '') {
      this.ngOnInit();
    }
    let postsRes: Post[] = [];
    for (let i = 0; i < this.posts.length; i++) {
      if (this.posts[i].theme == theme) {
        postsRes.push(this.posts[i]);
      }
    }
    this.posts = postsRes;
  }
  ...
  openEditDialog(id: number): void {
    const dialogAlarmConfig = new MatDialogConfig();
    dialogAlarmConfig.width = '400px';
    dialogAlarmConfig.data = {
      user: this.user,
    };
    this.dialog.open(AlarmComponent, dialogAlarmConfig);
  }
}
